...
class SystemService extends Service
{
    ...
    public function set(string $name, $value = '')
    {
        ...
        if (is_array($value)) {
            ...
            $this->app->cache->delete($this->table . SITE_ID);
            $map = ['site_id' => SITE_ID, 'type' => $type, 'name' => $field];
    ...
    {
        $val = $data[$key] ?? null;
        $data['site_id'] = $site_id = $data['site_id'] ?? SITE_ID;
        $data['create_at'] = time();
        $query = (is_string($dbQuery) ? $this->app->db->name($dbQuery) : $dbQuery)->master()->strict(false)->where($where);
        if (empty($where[$key])) is_string($val) && strpos($val, ',') !== false ? $query->whereIn($key, explode(',', $val)) : $query->where([$key => $val, 'site_id' => $site_id]);
    ...
    public function setRuntime(?string $mode = null, ?array $appmap = [], ?array $domain = []): bool
    {
        ...
        $data['mode'] = env('app_debug') ? 'debug' : 'product';
