...
class QueueService extends Service
{
    ...
    public function initialize($code = 0): QueueService
    {
        if (!empty($code)) {
            ...
            [$this->site_id, $this->code, $this->title] = [$this->record['site_id'], $this->record['code'], $this->record['title']];
            if (empty(defined('SITE_ID'))) define('SITE_ID', $this->site_id);
    ...
    {
        $site_id = defined('SITE_ID') ? SITE_ID : 0;
     ...
     * @param null|float|string $progress 进度数值
     * @param integer $backline 回退信息行
     * @return array
     */
    public function progress(?int $status = null, ?string $message = null, $progress = null, int $backline = 0): array
    {
     ...
     */
    public function message(int $total, int $used, string $message = '', int $backline = 0): void
    {
